...
export class partyController {
  ...
  createParty = asyncHandler(async (req, res) => {
    const data = req.body;
    try {
      ...
      if (err.code === 11000) {
        return res
          .status(409)
          .json(
            new ApiResponse(409, null, `${data?.first_name} already exists.`)
          );
      }
      throw new ApiError(
        500,
  ...
  updateParty = asyncHandler(async (req, res) => {
    try {
      const { id } = req.params;
  ...
  removeParty = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const find = await this.Model.findById(id);
